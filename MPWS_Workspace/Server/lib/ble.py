...
class BleServer(Bluetooth):
    ...
    def start_new_proto_choice_thread(self, server_data):
        _thread.start_new_thread(self.send_new_proto_choice, (server_data,))


    def send_new_proto_choice(self, server_data):
        # Wait till confirmation that clients get the new protocol choice
        while True:
            if server_data.protocol != 'BLE': # stop thread
                print('Send new proto to BLE clients')
                self.chr_proto_lp.value(server_data.protocol)
                self.chr_proto_am.value(server_data.protocol)
                while self.am2320_proto_conf == False or self.lps25hb_proto_conf == False:
                    print('BLE clients confirmed new protocol choice')
                    self.am2320_proto_conf = False
                    self.lps25hb_proto_conf = False
                break
